...
const AlunoEventos: FC = () => {
  ...
  const fetchEventos = async () => {
    try {
      ...
      const url = `${connectionUrl}/aluno/eventos${mostrarAntigos ? '?futuros=false' : '?futuros=true'}`;
  ...
  const verificarPresencaConfirmada = async (eventoId: number) => {
    try {
      ...
      const token = localStorage.getItem('token');
      console.log(`Verificando presença: ${connectionUrl}/eventos/${eventoId}/confirmado`);
      
      const response = await fetch(`${connectionUrl}/eventos/${eventoId}/confirmado`, {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      });
      ...
      if (response.ok) {
        ...
        // Tenta ler o corpo da resposta para mais informações
        try {
  ...
  const confirmarPresenca = async () => {
    try {
      ...
      setLoadingConfirmacao(true);
      const token = localStorage.getItem('token');
      console.log(`Confirmando presença: ${connectionUrl}/eventos/${selectedEvento.id}/confirmar`);
      
      const response = await fetch(`${connectionUrl}/eventos/${selectedEvento.id}/confirmar`, {
        method: 'POST',
        headers: {
